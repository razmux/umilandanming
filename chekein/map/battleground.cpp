...
int bg_send_xy_timer_sub(DBKey key, DBData *data, va_list ap)
{
	...
	for( i = 0; i < MAX_BG_MEMBERS; i++ )
	{
		...
		{
			sprintf(output, "- AFK [%s] Exclu -", sd->status.name);
			clif_broadcast2(&sd->bl, output, (int)strlen(output)+1, bg->color, 0x190, 20, 0, 0, BG);

			bg_team_leave(sd,3);
			clif_displaymessage(sd->fd, "Vous etes exclu du battleground en raison d'une trop longue inactivite.");
		...
		{ // Idle announces
			sd->state.bg_afk = 1;
			sprintf(output, "%s : %s semble etre AFK - Il peut etre exclu avec @reportafk", bg->g->name, sd->status.name);
